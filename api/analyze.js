...
function supportsNoneReasoning(model = "") {
  return /^gpt-5\.1/i.test(String(model || "").trim());
}

function hasMeaningfulAtsData(data, isPreview) {
  const hasSummary = typeof data?.summary === "string" && data.summary.trim().length > 0;
  const hasKeywords = Array.isArray(data?.missing_keywords) && data.missing_keywords.length > 0;
  const hasWeak = Array.isArray(data?.weak_sentences) && data.weak_sentences.length > 0;
  const hasCv =
    !isPreview &&
    typeof data?.optimized_cv === "string" &&
    data.optimized_cv.trim().length > 0;

  return hasSummary || hasKeywords || hasWeak || hasCv;
}
...
async function callOpenAIJson({
  ...
  const useGpt5 = isGpt5Family(model);
  ...
  if (useGpt5) {
    ...
    // GPT-5.1'de temperature sadece reasoning_effort = none iken desteklenir.
    if (supportsNoneReasoning(model) && reasoningEffort === "none") {
  ...
  if (choice?.finish_reason === "length") {
    const err = new Error("Model output was cut off before producing usable JSON");
    err.status = 502;
    err.details = JSON.stringify({
...
WEAK SENTENCES:
- weak_sentences SHOULD include 6–10 high-quality items from the weakest, most improvable resume lines.
- Return fewer if fewer strong rewrite candidates exist.
- Both sentence and rewrite MUST be in ${outLang}.
...
`.trim();
    ...
    try {
      data = await callOpenAIJson({
        ...
        system: chosenSystem,
        userPrompt,
        reasoningEffort: isPreview ? "medium" : "high",
        maxCompletionTokens: isPreview ? 4500 : 9000,
        temperature: 0.1,
    ...
    const derivedScore = computeWeightedScore(componentScores, hasJD);
    ...
    if (!hasMeaningfulAtsData(normalized, isPreview)) {
      return res.status(502).json({
        error: "Model returned incomplete structured output",
        details: "Empty or unusable ATS payload received from model",
      });
    }
