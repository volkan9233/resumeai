...
function buildWeakRewriteFallbackPrompt({ cv, jd, hasJD, candidates, outLang, roleProfile }) {
  ...
  return [
    ...
    "\nSTRICT RULES:",
    "- A rewrite that is only more formal is NOT enough; it must be materially stronger.",
    "- Reject shallow rewrites where only 1-2 words change or the sentence meaning stays almost the same.",
    "- Reject weak verb swaps such as helped to assisted, supported to contributed, worked on to participated in, or handled to managed when the sentence remains essentially the same.",
    "- Each rewrite must improve at least 2 of these: action strength, task clarity, scope definition, role-specific terminology, process or workflow context, recruiter readability.",
    "- Do not keep weak or support-style wording at the start of the rewrite.",
    "- Do not use awkward phrasing or add fake impact, fake ownership, fake tools, fake metrics, or fake business results.",
...
function buildRepairPrompt({ cv, jd, hasJD, currentOptimizedCv, summary, missingKeywords, bulletUpgrades, unsupportedTerms = [], outLang, roleProfile }) {
  ...
  return [
    ...
    "- Do not keep weak or support-style wording at the start of the rewrite.",
    "- Do not use awkward phrasing or add fake impact, fake ownership, fake tools, fake metrics, or fake business results.",
    "- Keep the language native to the job family and do not turn specialized work into generic business wording.",
    `\nROLE CONTEXT:\n${roleContextText}`,
    hasJD ? `\nRANKED JD SIGNALS:\n${buildJdSignalText(jd, roleProfile, cv)}` : "",
    `\nALLOWED EXPLICIT TOOLS / PLATFORMS / ACRONYMS:\n${allowedTermsText}`,
